...
class ConfiguratorController < ApplicationController
  ...
  def step2_submit
    ...
    session[:budget] = if params[:budget].present?
                         params[:budget]
                       else
                         10_000
                       end
  ...
  def results
    ...
    @ready_ram.each do |ram|
      ...
      next if @ready_motherboard.empty?
    ...
    missing_components << 'Power Supply' if @top_1_power_supply.empty? || @top_1_power_supply.zero?
    ...
    @added_price = @top_1_cooler[0]&.price || (0 +
      @top_1_rig[0]&.price) || 0
